...
module.exports = {
  ...
  run: async (client, lang, interaction, options) => {
    ...
    .addField(cmdLang.support.title, cmdLang.support.value+process.env.INVITE_SUPPORT+')', true)
    .addField(cmdLang.invite.title, cmdLang.invite.value+process.env.INVITE_URL+')', true)
    .addField(cmdLang.source.title, cmdLang.source.value+process.env.SOURCE_URL+')', true)
    interaction.reply({
      embeds: [embed],
      ephemeral: true
    })
